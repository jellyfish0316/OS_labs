...
struct inode *osfs_new_inode(const struct inode *dir, umode_t mode)
{
    ...
    /* Allocate data block */
    ret = osfs_alloc_data_block(sb_info, &osfs_inode->i_block[0]);//Block 編號填進osfs_inode->i_block[0]
    if (ret) {
...
{   
    ...
    // We check against MAX_FILENAME_LEN (defined in osfs.h)
    ...
    // Add the new file's name and inode number to the parent directory's data block
    ...
    // 2. 更新 VFS 層的父目錄 Inode (讓 ls 指令看得到)
    inode_set_mtime_to_ts(dir, now);
    inode_set_ctime_to_ts(dir, now);
    // (atime 通常在讀取時更新，create 時可選，但 mtime/ctime 必更)

    // 3. 同步更新底層 OSFS 層的父目錄 Inode (寫回儲存裝置用)
    parent_inode->__i_mtime = now;
    parent_inode->__i_ctime = now;
    
    // Step 6: Bind the inode to the VFS dentry
    // This connects the dentry (filename) to the new inode (metadata)
